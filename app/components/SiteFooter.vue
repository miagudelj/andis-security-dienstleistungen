...
<template>
  <footer class="mt-12 border-t border-ink-100 bg-ink-900 text-ink-100">
    <div class="container grid items-start gap-8 py-12 md:grid-cols-5">
      <div class="md:col-span-2">
        <img src="/images/presecurity-logo.png" alt="PreSecurity" class="max-h-36 w-auto" />
      </div>

      <div class="pt-1">
        <h4 class="text-sm font-semibold text-white">{{ t('footer.company') }}</h4>
        <ul class="mt-3 space-y-2 text-sm">
          <li><NuxtLink :to="localePath('/ueber-uns')" class="text-ink-300 hover:text-white">{{ t('nav.about') }}</NuxtLink></li>
          <li><NuxtLink :to="localePath('/dienstleistungen')" class="text-ink-300 hover:text-white">{{ t('nav.services') }}</NuxtLink></li>
          <li><NuxtLink :to="localePath('/kontakt')" class="text-ink-300 hover:text-white">{{ t('nav.contact') }}</NuxtLink></li>
        </ul>
      </div>
      ...
      <div class="pt-1">
        ...
        <ul class="mt-3 space-y-2 text-sm text-ink-300">
          ...
          <li v-if="settings?.street || settings?.zip || settings?.city">
            <template v-if="settings?.street">{{ settings.street }}, </template>{{ settings?.zip }} {{ settings?.city }}<template v-if="settings?.canton">, {{ t('common.canton') }} {{ settings.canton }}</template>
          </li>
    ...
    <div class="border-t border-ink-800 py-5">
      <div class="container text-center text-xs text-ink-400">
