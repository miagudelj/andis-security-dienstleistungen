  ...
  try {
    const result = db.prepare(`
      UPDATE wizard_steps SET
        slug = ?, step_type = ?, title_de = ?, title_en = ?, subtitle_de = ?, subtitle_en = ?,
        error_message_de = ?, error_message_en = ?, is_required = ?, min_selections = ?, max_selections = ?,
    ...
    `).run(
      ...
      data.sort_order, data.active ? 1 : 0, id
    )
    ...
    logAudit('wizard_step_updated', data.slug, '', ipHash)
